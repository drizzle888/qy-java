...
public class SnowJob implements Job {
	private final static Logger logger = LoggerFactory.getLogger(SnowJob.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		roleList.addAll(attRole.visionRoleList);
		boolean isHit;
		for (Iterator<Role> it = roleList.iterator(); it.hasNext();) {
			...
			if (gameService.isAttackGroup(attRole, beRole, skillTemplate)) {
				...
				logger.info(String.format("role.id=%s location=%s isHit=%s", beRole.id, beRole.location, isHit));
				if (isHit) {
