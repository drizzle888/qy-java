...
<html>
	...
	<body>
		...
		<nav class="breadcrumb"> <i class="Hui-iconfont">&#xe67f;</i> 首页
			...
			<span class="c-gray en">&gt;</span> 牌局明细
		...
		<div class="pd-20">
			<div class="text-c">
				...
				<input type="text" name="roundCount" id="roundCount" placeholder=" 局数Id" style="width:300px" class="input-text" value="${roundCount}">
			...
			<div class="mt-20">
				<table id="roundResultTable" class="table table-border table-bordered table-bg table-hover table-sort">
					<thead>
						<tr class="text-c">
							<th>序号</th>
							<th>房间Id</th>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				roundResultDT = $("#roundResultTable").DataTable({
					'ajax': {
						'url': $("#contextPath").val() + '/roundResult/list.do',
						'data': function(data) {
		...
		/*	function _add() {
			...
			function edit(id) {
				var title = "编辑";
				var url = $("#base").val() + '/roundResult/goEdit.do?id=' + id;
				layer_show(title, url, '', '');
			}
			...
			function browse(id) {
				...
				var title = "查看";
				var url = $("#base").val() + '/roundResult/goView.do?id=' + id;
				layer_show(title, url, '', '');
			}
			...
			function del(salf, id) {
				...
				layer.confirm('确认要删除吗？', function(index) {
					var url = $("#base").val() + '/roundResult/delete.do?id=' + id;
					$.ajax({
						type: 'POST',
						url: url,
						dataType: 'json',
