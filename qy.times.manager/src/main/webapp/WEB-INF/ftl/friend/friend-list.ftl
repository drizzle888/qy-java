...
<html>
	...
	<body>
		...
		<nav class="breadcrumb"> <i class="Hui-iconfont">&#xe67f;</i> 首页
			...
			<span class="c-gray en">&gt;</span> 好友列表
		...
		<div class="pd-20">
			<div class="text-c">
				...
				<input type="text" name="memberAlias" id="memberAlias" placeholder=" 成员别名" style="width:300px" class="input-text" value="${memberAlias}">
			...
			<div class="mt-20">
				<table id="friendTable" class="table table-border table-bordered table-bg table-hover table-sort">
					<thead>
						<tr class="text-c">
							<th>序号</th>
							<th>成员Id</th>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				friendDT = $("#friendTable").DataTable({
					'ajax': {
						'url': $("#contextPath").val() + '/friend/list.do',
						'data': function(data) {
							var memberId = $('#memberId').val();
							var memberAlias = $('#memberAlias').val();
							data.memberId = memberId;
							data.memberAlias = memberAlias;
						},
					},
					'columns': [{
						'data': 'id'
						...
						render: function(data, type, row, meta) {
							return '<div id="preview" ><img class="imgMember" src="' + data + '" /></div>';
						}
					}, {
						'data': 'friendId'
					}, {
						...
						'data': 'friendAvatar',
						render: function(data, type, row, meta) {
							return '<div id="preview" ><img class="imgFriend" src="' + data + '" /></div>';
						}
					}, /*{
						'data': 'status'
					},*/ {
		...
		/*	function _add() {
			...
			function edit(id) {
				var title = "编辑";
				var url = $("#base").val() + '/friend/goEdit.do?id=' + id;
				layer_show(title, url, '', '');
			}
			...
			function browse(id) {
				...
				var title = "查看";
				var url = $("#base").val() + '/friend/goView.do?id=' + id;
				layer_show(title, url, '', '');
			}
			...
			function del(salf, id) {
				...
				layer.confirm('确认要删除吗？', function(index) {
					var url = $("#base").val() + '/friend/delete.do?id=' + id;
					$.ajax({
						type: 'POST',
						url: url,
						dataType: 'json',
