...
<html>
	...
	<body>
		...
		<nav class="breadcrumb"> <i class="Hui-iconfont">&#xe67f;</i> 首页
			...
			<span class="c-gray en">&gt;</span> 账户每天局数
		...
		<div class="pd-20">
			<div class="text-c">
				...
				<!--<input type="hidden" name="name" id="name" placeholder=" 成员名称" style="width:300px" class="input-text" value="${name}">-->
			...
			<div class="mt-20">
				<table id="roundTable" class="table table-border table-bordered table-bg table-hover table-sort">
					<thead>
						<tr class="text-c">
							<th>成员ID</th>
							<th>时间</th>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				//debugger;
				rountDT = $("#roundTable").DataTable({
					'ajax': {
						'url': $("#contextPath").val() + '/account/roundList.do',
						'data': function(data) {
			...
			function edit(memberId) {
				var title = "编辑";
				var url = $("#base").val() + '/account/goRoundEdit.do?memberId=' + memberId;
				layer_show(title, url, '', '');
			}
			...
			function browse(memberId) {
				...
				var title = "查看";
				var url = $("#base").val() + '/account/goRoundView.do?memberId=' + memberId;
				layer_show(title, url, '', '');
			}
			...
			function del(salf, memberId) {
				...
				layer.confirm('确认要删除吗？', function(index) {
					var url = $("#base").val() + '/account/roundDelete.do?memberId=' + memberId;
					$.ajax({
						type: 'POST',
						url: url,
						dataType: 'json',
